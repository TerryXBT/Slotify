...
// date-fns imports available for future test expansions
...
function _createMockQueryBuilderForList(
  data: unknown[],
  error: unknown = null,
) {
  return {
...
describe("getAvailableSlots", () => {
  ...
  describe("Provider and Service Validation", () => {
    it("should throw error when provider not found", async () => {
      ...
      await expect(
        getAvailableSlots(mockUsername, mockServiceId, "2025-01-20"),
        ...
        if (table === "profiles") {
          return {
            ...
            single: vi
              .fn()
              .mockResolvedValue({
                data: { id: mockProviderId, timezone: "UTC" },
                error: null,
              }),
          };
        }
  ...
  describe("Slot Generation Logic", () => {
        ...
        if (table === "profiles") {
          return {
            ...
            single: vi
              .fn()
              .mockResolvedValue({
                data: { id: mockProviderId, timezone },
                error: null,
              }),
          };
        }
        ...
        rules: [
          {
            day_of_week: 1,
            start_time_local: "09:00:00",
            end_time_local: "10:30:00",
          },
        ],
        bookings: [
          { start_at: "2025-01-20T09:30:00Z", end_at: "2025-01-20T10:00:00Z" },
        ],
      });
    ...
    it("should exclude slots that conflict with busy blocks", async () => {
      setupStandardMocks({
        ...
        busyBlocks: [
          { start_at: "2025-01-20T09:00:00Z", end_at: "2025-01-20T10:00:00Z" },
        ],
      });
    ...
    it("should apply buffer times correctly", async () => {
      ...
      setupStandardMocks({
        ...
        bookings: [
          { start_at: "2025-01-20T10:00:00Z", end_at: "2025-01-20T10:30:00Z" },
        ],
      });
      ...
      const _slotTimes = slots.map((s) => new Date(s.start).toISOString());
  ...
  describe("Conflict Detection Algorithm", () => {
    it("should correctly identify overlapping time ranges", async () => {
      ...
      const _slotStartHours = slots.map((s) => new Date(s.start).getUTCHours());
