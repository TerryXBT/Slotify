...
describe('createBookingAction', () => {
    ...
    describe('Successful Booking Creation', () => {
        ...
        it('should generate cancel token that expires 24h before appointment', async () => {
            ...
            mockFrom.mockImplementation((table: string) => {
                if (table === 'action_tokens') {
                    return {
                        insert: vi.fn().mockImplementation((data: { expires_at: string }) => {
                            insertedToken = data
            ...
            const expectedExpiry = new Date(appointmentTime - 24 * 60 * 60 * 1000).toISOString()
    ...
    describe('Error Handling', () => {
        ...
        it('should continue if token creation fails', async () => {
            ...
            mockFrom.mockImplementation((table: string) => {
                if (table === 'action_tokens') {
                    return {
                        insert: vi.fn().mockResolvedValue({ error: { message: 'Token error' } })
                    }
                }
                if (table === 'bookings') {
                    return {
        ...
        it('should return error when booking fetch fails', async () => {
            ...
            expect(result.error).toBe('Booking created but failed to retrieve details')
        })
    ...
    describe('Double Booking Prevention', () => {
        it('should use atomic RPC to prevent race conditions', async () => {
            ...
            const hasDirectInsert = insertCalls.some((call: string[]) => {
