...
// ProfileUpdate type available for future use
...
export async function updateProfile(formData: FormData) {
  ...
  const _location = formData.get("location") as string | null; // reserved for future migration
  const phone = formData.get("phone") as string | null;
  const _email = formData.get("email") as string | null; // lives in auth.users, not profiles
  const cancellationPolicy = formData.get("cancellation_policy") as
    | string
    | null;
  ...
  if (cancellationPolicy)
    updateData.cancellation_policy_text = cancellationPolicy;
...
export async function createService(formData: FormData) {
  ...
  const { data: insertedService, error: insertError } = await (
    adminClient as any
  )
    .from("services")
    .insert({
  ...
  if (
    insertedService?.id &&
    (priceNegotiable || buffer_minutes || cancellation_policy)
  ) {
...
export async function updateService(id: string, formData: FormData) {
  ...
  await fetch(
    `${supabaseUrl}/rest/v1/services?id=eq.${id}&provider_id=eq.${user.id}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export async function restoreService(id: string) {
  try {
    ...
    return {
      error: "An unexpected error occurred while restoring the service",
    };
  }
}
...
export async function permanentlyDeleteService(id: string) {
  try {
    ...
    if (bookingsError) {
      ...
      return {
        error: `Failed to delete related bookings: ${bookingsError.message}`,
      };
    }
    ...
    if (serviceError) {
      ...
      return {
        error: `Failed to permanently delete service: ${serviceError.message}`,
      };
    }
    ...
    return {
      error:
        "An unexpected error occurred while permanently deleting the service",
    };
  }
}
...
export async function updateDefaultBookingSettings(formData: FormData) {
  try {
    ...
    const default_buffer_minutes =
      parseInt(formData.get("default_buffer_minutes") as string) || 0;
    const default_cancellation_policy =
      (formData.get("default_cancellation_policy") as string) || "24h";
