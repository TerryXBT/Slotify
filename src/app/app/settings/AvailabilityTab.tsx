...
import { ChevronLeft, Plus, Trash2, Copy, Check, X } from "lucide-react";
import {
...
export default function AvailabilityTab({
  ...
  const hasChanges = useMemo(() => {
    ...
    for (const rule of localRules) {
      ...
      if (original.start_time_local.slice(0, 5) !== rule.start_time)
        return true;
  ...
  const handleAddRule = (dayId: number) => {
    const dayRules = localRules
      .filter((r) => r.day_of_week === dayId)
    ...
    if (dayRules.length > 0) {
      ...
      const [h, _m] = start.split(":").map(Number);
      let endH = h + 1;
      if (endH >= 24) endH = 23;
  ...
  const handleApplyCopy = () => {
    ...
    const sourceRules = localRules.filter(
      (r) => r.day_of_week === copySourceDay,
    );
    const newRules = localRules.filter(
      (r) => !selectedCopyDays.includes(r.day_of_week),
    );
  ...
  const getDayStatusText = (dayId: number) => {
    ...
    if (rules.length === 1)
      return formatTimeRange(rules[0].start_time, rules[0].end_time);
  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] text-white font-sans selection:bg-blue-500/30 pb-20">
      ...
      <div className="px-4 pt-14 pb-2 flex items-center justify-between bg-[#1a1a1a]/80 backdrop-blur-md sticky top-0 z-10 border-b border-white/5">
        <Link
          href="/app/services"
          className="flex items-center gap-1 text-blue-500 active:opacity-50 transition-opacity -ml-2 p-2"
        >
          ...
          <div className="relative z-10 divide-y divide-white/5">
            {DAYS.map((day) => {
              ...
              return (
                ...
                >
                  <span className="text-[17px] text-white font-medium w-24">
                    {day.label}
                  </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-2xl shadow-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
            ...
            <div className="relative z-10 p-4 space-y-4 max-h-[70vh] overflow-y-auto">
              {localRules.filter((r) => r.day_of_week === editingDayId)
                .length === 0 ? (
      ...
      {copySheetOpen && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center">
          <div
            ...
            onClick={() => setCopySheetOpen(false)}
          />
          <div className="relative w-full max-w-md rounded-t-2xl shadow-2xl animate-in slide-in-from-bottom duration-300 overflow-hidden">
            ...
            <div className="relative z-10 px-4 py-3 border-b border-white/10 flex items-center justify-between">
              <span className="text-[17px] font-semibold text-white">
                Copy {DAYS.find((d) => d.id === copySourceDay)?.label}&apos;s
                schedule to...
              </span>
              <button
                onClick={() => setCopySheetOpen(false)}
            ...
            <div className="relative z-10 p-4 space-y-1 max-h-[60vh] overflow-y-auto">
              {DAYS.filter((d) => d.id !== copySourceDay).map((day) => (
                <label
                  key={day.id}
                  className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 active:bg-white/10 cursor-pointer transition-colors"
                >
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked)
                        setSelectedCopyDays([...selectedCopyDays, day.id]);
                      else
                        setSelectedCopyDays(
                          selectedCopyDays.filter((id) => id !== day.id),
                        );
                    }}
                  />
                </label>
