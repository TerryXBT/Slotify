...
export default async function TodayPage() {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-black p-4">
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-2xl shadow-xl w-full max-w-md text-center space-y-4">
          ...
          <p className="text-gray-500">
            Your account exists but your profile is missing. This shouldn&apos;t
            happen.
          </p>
          <p className="text-gray-400 text-sm">
            Please logout and try signing up again, or contact support if the
            problem persists.
          </p>
          <div className="pt-4">
            <Link
  ...
  const { data: bookings } = await supabase
    .from("bookings")
    .select(
      "*, services(name, duration_minutes, location_type, default_location)",
    )
  ...
  const totalBookedMinutes =
    bookings?.reduce(
      (acc: number, b: BookingWithService) =>
        acc + (b.services?.duration_minutes || 0),
      0,
    ) || 0;
  ...
  return (
    <div className="min-h-screen bg-[#1a1a1a] font-sans pb-32 text-white selection:bg-blue-500/30">
      ...
      <header className="pt-14 pb-4 px-6 flex justify-between items-center bg-[#1a1a1a]/80 backdrop-blur-md sticky top-0 z-50 border-b border-white/5">
        ...
        <Link
          href="/app/settings"
          className="relative active:opacity-70 transition-all hover:scale-105"
        >
          {profile.avatar_url ? (
            <Image
        ...
        <section>
          ...
          {nextBooking ? (
            ...
            <div className="relative rounded-3xl py-12 flex flex-col items-center justify-center text-center overflow-hidden">
              ...
              <h3 className="relative z-10 text-lg font-semibold text-white mb-1">
                No upcoming bookings
              </h3>
              <p className="relative z-10 text-sm text-white/50 max-w-[200px] leading-relaxed">
