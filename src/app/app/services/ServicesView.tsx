...
export default function ServicesView({
  ...
  const [_isCreatingService, setIsCreatingService] = useState(false);
  ...
  // Share service link (used by share button in UI)
  const _handleShare = async (e: React.MouseEvent, service: Service) => {
  ...
  // Create new service (reserved for inline creation form)
  const _handleCreateService = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#1a1a1a] pb-24 font-sans selection:bg-blue-500/30">
      ...
      <main className="px-5 space-y-8">
                  ...
                  >
                    <div className="flex-1 min-w-0 pr-4">
                      ...
                      <p className="text-[15px] text-gray-500 font-normal">
                        ...
                        {((service.price_cents ?? 0) / 100).toFixed(0)}
                      </p>
                    </div>
        ...
        <section>
          <h2 className="text-[13px] text-gray-500 uppercase font-medium ml-4 mb-2">
            Business Profile
          </h2>
          <div className="relative rounded-2xl overflow-hidden">
            {/* Glassmorphism Background */}
            ...
            <div className="relative z-10 divide-y divide-white/5">
              ...
              >
                ...
                <div className="flex gap-1.5">
                  {DAYS.map((day) => {
                    const hasRules =
                      (rulesByDay.find((d) => d.id === day.id)?.rules.length ??
                        0) > 0;
                    return (
                      <div
                        key={day.id}
              ...
              <div
                onClick={() =>
                  window.open(`/${profile?.username || profile?.id}`, "_blank")
                }
                className="flex items-center justify-between px-4 py-4 active:bg-gray-800/50 cursor-pointer group transition-colors"
              >
                <span className="text-[17px] text-white font-medium">
            ...
            <div className="relative rounded-2xl overflow-hidden">
              ...
              >
                <span className="text-[15px] text-white/70 font-medium">
                  Recently Deleted ({deletedServices.length})
                </span>
                <ChevronDown
                  className={clsx(
              ...
              {showTrash && (
                <div className="relative z-10 border-t border-white/10">
                  {deletedServices.map((service: Service) => (
                    ...
                    >
                      <div>
                        <h4 className="text-[15px] text-gray-500 line-through">
                          {service.name}
                        </h4>
                        <p className="text-[13px] text-gray-600">
                          Deleted{" "}
                          {service.deleted_at
                            ? new Date(service.deleted_at).toLocaleDateString()
                            : "recently"}
                        </p>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          onClick={() =>
                            openPermanentDeleteModal(service.id, service.name)
                          }
                          className="p-2 hover:bg-gray-800 rounded-full transition-colors"
                        >
          ...
          <div className="relative rounded-2xl p-6 w-full max-w-[320px] shadow-2xl overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              <h3 className="text-[17px] font-bold text-white mb-2">
                Delete &quot;{permanentDeleteModal.serviceName}&quot;?
              </h3>
